...
export const HeaderColumnToggle = ({ api: gridApi, columnApi }: HeaderColumnToggleProps)=> {
  ...
  const setVisibleColumns = () => {
      const visibleColumnIds = columnApi.getAllColumns()?.filter((x) => x.isVisible()).map((x) => x.getColId());
  ...
  useEffect(() => {
    const items = columnApi.getAllColumns()?.filter((c => c.getColDef().headerName)).map((c) => 
      ({
          id: c.getColId(),
